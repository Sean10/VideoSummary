...
- 视频总结
date: 2023-03-06
subtitle: Ceph_Performance_Meeting_2023-03-02
tags:
- Ceph
- 性能优化
- RocksDB
...
本次会议于2023年3月2日召开，主要回顾了最近两周的Pull Request（PR）审查进度，并讨论了多个关键议题，包括RocksDB更新、PR审查、新的基准测试工具、读取平衡器性能改进以及QAT加密性能问题。会议还决定了后续行动计划。

**主要讨论议题**

1. **PR审查进度**
   - 主持人接近完成PR列表的审查，但尚未完全完成。
   - 讨论了Casey关于在rgw中切换回多态执行器的PR，Mark Hogan进行了测试，未发现明显的性能提升，但仍计划在Reef之后进行切换。

2. **已关闭的PR**
   - 讨论了Igor的一个csq更改，使用更大的读取块在cue列表条目中，预计会有小的性能改进。
   - 讨论了关于更新Facebook的RocksDB的PR。

3. **新的和更新的PR**
   - 讨论了替换RBD配置的PR，加密添加qat批处理的PR等。
