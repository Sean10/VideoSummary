...
categories:
- "视频总结"
subtitle: "Ceph Orchestrator Meeting 2021-09-07"
---
...
- **架构**：使用HTTP端点和非容器化的systemd单元，通过原始套接字通信。
- **安全性**：使用HTTPS、自签名证书和密钥环确保通信安全。
- **元数据完整性**：使用计数器确保元数据的时效性。
- **离线主机处理**：通过超时机制快速检测离线主机，并重新部署Agent。

##### 2. Repaving Oasis
- **目的**：自动化OSD的重铺过程，例如更改最小分配大小或使用db wal。
- **方法**：使用现有的驱动组定义来重新部署OSD。
- **命令**：设想一个新的命令来启动重铺过程，例如`ceph osd repave`。
- **测试**：需要验证在混合OSD（SSD和硬盘）情况下的正确行为。

##### 3. 测试重启
- **目的**：避免在重启过程中出现Monmap问题。
- **方法**：使用现有的压力测试工具（如Thrasher）进行测试。

##### 4. 日志聚合需求
- **目的**：聚合集群中的日志，可能用于支持案例。
- **讨论**：建议使用外部工具（如EFK堆栈）来处理日志聚合。
