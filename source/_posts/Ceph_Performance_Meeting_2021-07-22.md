...
- 视频总结
date: 2021-08-23
subtitle: Ceph_Performance_Meeting_2021-07-22
tags:
- Ceph
- 分布式存储
- 性能优化
...
本次会议主要讨论了Ceph项目的多个Pull Requests (PRs)，包括新提交、更新和关闭的PRs，以及Ceph的性能改进、内存分配优化和对未来存储系统发展方向的讨论。

### 主要议题

1. **新PR和更新PRs**
   - 一个新的PR改变了vlog的工作方式，Kifu已批准并要求Sam进行审查。
   - Radic提交了一个PR，允许Crimson使用外来的存储（alien store），以测试mem store和file store。
   - 新的PR用于RGW跟踪，Matt已进行审查。
   - 针对c字符串转换的buffer list优化。
   - 一个PR改进了finisher线程的CPU占用，正在审查中。
