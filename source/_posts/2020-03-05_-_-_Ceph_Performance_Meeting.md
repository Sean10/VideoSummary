...
- 视频总结
date: 2020-04-03
subtitle: 2020-03-05_-_-_Ceph_Performance_Meeting
tags:
- Ceph
- 性能优化
- 分布式存储
...
2. **IO 500测试进展**
   ...
   - 开发了一个后端用于IOR和MD测试，直接与libcephfs集成，初步结果显示在大尺寸顺序读取上显著更快。

3. **PG日志更新优化**
   - 讨论了移除PG日志调用的实验，结果显示延迟显著降低，IOPS每核心有所提升。
   - 建议将PG日志更新从BlueStore移至其他不引起大量光放大的地方。

#### 决定事项：
- 继续优化和测试IO 500性能，特别是比较内核客户端和libcephfs的性能。
- 进一步研究和实施PG日志更新的优化策略。
...
#### 其他讨论：
- 讨论了使用不同客户端（如Fuse和Loops CephFS）的性能差异及其原因。
