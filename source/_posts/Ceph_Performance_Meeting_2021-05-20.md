...
- 会议纪要
date: 2021-05-20
subtitle: Ceph_Performance_Meeting_2021-05-20
tags:
- Ceph
- 分布式存储
- 性能优化
...
在2021年5月20日的Ceph性能会议上，讨论了多个与Ceph性能优化和数据收集相关的重要议题。

**关键细节**：
- 本周没有新的性能相关PR。
- 本周关闭了两个PR，一个针对Crimson的优化，涉及客户端请求处理并行性；另一个是关于自动调优，让管理器自动设置容器内存限制。

**讨论的主要议题**：
- **自动调优**： 讨论了自动调优的实现细节，特别是如何根据某些比例自动设置容器内存限制。
...
会议还讨论了内存使用、数据隐私和安全性等问题。通过这些讨论和决策，Ceph社区将继续推动项目的改进和发展。