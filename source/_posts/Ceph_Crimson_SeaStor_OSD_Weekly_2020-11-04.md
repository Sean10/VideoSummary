...
categories:
- "视频总结"
subtitle: Ceph_Crimson_SeaStor_OSD_Weekly_2020-11-04
---
...
1. **对象上下文锁定恢复问题**：
   - 讨论了PG活动集更改时解锁对象上下文的问题。
   - 提出了使用`with lock`原语确保解锁操作总是被调用的解决方案。
...
3. **PG状态修复**：
   - 讨论了遇到EIO时将PG状态设置为修复的原因和后果。
   - 确认修复状态是为了触发后续的scrub和修复操作。

4. **文档编写**：
   - 建议编写文档以解释设计目标和计划的上传位置，以帮助评审人员理解。
