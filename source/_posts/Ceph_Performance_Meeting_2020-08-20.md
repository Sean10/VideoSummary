...
- 视频总结
date: 2020-08-20
subtitle: Ceph_Performance_Meeting_2020-08-20
tags:
- Ceph
- CRUSH算法
- 分布式存储
...
#### 会议概述
本次Ceph性能会议主要讨论了与Ceph性能相关的几个拉取请求，深入探讨了关于CRUSH算法扩展的论文，并对后续行动计划和潜在研究方向进行了讨论。

#### 讨论的主要议题
1. **拉取请求更新**：
   - 目前没有新的与性能相关的拉取请求。
   - majianpeng提交的一个拉取请求因长时间未被审查而被stale bot关闭。
   - majianpeng提交的另一个拉取请求已由adam和会议参与者审查，主要涉及bluerock环境的刷新行为更改。

2. **论文讨论**：
   - 讨论了一篇关于CRUSH算法扩展的论文，旨在解决集群扩展时数据迁移的问题。
   - 论文通过引入时间维度映射，使得在集群扩展时大部分数据不需要迁移，仅写入新的OSDs。
   - 论文还讨论了通过层合并来解决潜在的负载平衡问题。
