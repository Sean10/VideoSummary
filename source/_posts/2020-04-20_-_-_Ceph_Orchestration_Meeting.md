...
- 视频总结
date: 2020-04-29
subtitle: 2020-04-20_-_-_Ceph_Orchestration_Meeting
tags:
- Ceph
- Distributed Storage
- CRUSH Algorithm
...
1. **Ceph 15.2.2 版本更新**
   - 更新包括大量针对用户当前遇到问题的 bug 修复。
   - 改进了文档，帮助用户解决未预料到的问题和可用性问题。

2. **主机移除问题**
   - 讨论了从 Ceph 集群中移除主机的用户体验问题，指出当前流程繁琐且容易出错。
   - 发现调度器行为存在意外情况，例如在指定主机和数量时，调度器行为不符合预期。
...
本次 Ceph 每周会议的纪要涵盖了会议的关键细节、讨论的主要议题、决定的事项以及后续的行动计划，确保了 Ceph 分布式存储系统的持续优化和改进。