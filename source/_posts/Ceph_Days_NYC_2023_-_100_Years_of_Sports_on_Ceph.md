...
- 视频总结
date: 2023-05-17
subtitle: Ceph_Days_NYC_2023_-_100_Years_of_Sports_on_Ceph
tags:
- Ceph
- 分布式存储
- 体育数据存储
...
#### 会议概要：
本次会议介绍了基于Ceph的体育数据存储项目，旨在处理和存储大量体育视频数据，确保数据的安全、可访问性和长期保存。项目分为多个阶段，第一阶段计划构建40PB的Active Archive，并采用Ceph作为核心存储技术。

#### 讨论的主要议题：
1. **项目背景与动机**：
   - 项目源于对大量体育视频数据的存储需求，目前这些数据存储在磁带上，数据量不断增长。
...
#### 结论：
本次会议详细介绍了基于Ceph的体育数据存储项目的背景、实施细节和面临的挑战，展示了项目在技术选型、经济性和可靠性方面的优势，并明确了后续的行动计划。