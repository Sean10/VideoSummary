...
categories:
- "视频总结"
subtitle: Morning_Lightning_Talks_Ceph_Days_London_2024
---
...
* **Sanger Institute简介**： Sanger Institute是英国人类基因组计划的贡献者，专注于基因组研究和测序。
* **Ceph存储应用**： Sanger Institute使用Ceph存储进行数据存储，包括OpenStack私有云存储、S3对象存储和数据传输。
* **Ceph存储优势**： Ceph存储具有高可靠性、可扩展性和性能优势。
* **Ceph存储挑战**： 在使用过程中遇到了OSD崩溃、性能瓶颈和监控不足等问题。

**二、Ceph存储优化方案**

* **解决OSD崩溃**： 通过将垃圾回收池迁移到NVMe存储来解决OSD崩溃问题。
* **性能优化**： 通过调整Ceph配置和优化工作负载来提高Ceph存储性能。
* **监控和可观测性**： 实施新的监控系统以更好地监控Ceph存储性能。
...
* **Crimson优化**： 计划进一步优化Crimson的性能，例如调整CPU核心分配和优化Ceph配置。
