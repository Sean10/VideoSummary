...
- 视频总结
date: 2019-12-17
subtitle: 2019-12-17_-_Crimson_SeaStor_OSD_Weekly_Meeting
tags:
- Ceph
- BlueStore
- 性能优化
...
1. **Ceph BlueStore性能问题**:
   - **问题描述**: Crimson OSD在特定工作负载下性能不如Classic OSD。
   - **原因分析**: 发现与Ceph的分配器有关，特别是使用C-star分配器时无法分配特定页码导致错误。
   - **解决方案**: 建议使用默认分配器，并考虑增加页码限制或调整分配策略。

2. **提交策略改进**:
   - **建议**: 提交代码时应详细说明变更策略和具体改动。
   - **行动**: 将在提交中添加更多注释和详细说明。

3. **性能回归问题**:
   - **讨论**: 块大小大于4K时性能下降是预期的，需要通过改进策略来解决。
