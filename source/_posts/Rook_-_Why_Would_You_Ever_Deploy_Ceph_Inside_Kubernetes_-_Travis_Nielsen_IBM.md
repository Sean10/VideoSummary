...
- 视频总结
date: 2023-05-05
subtitle: Rook_-_Why_Would_You_Ever_Deploy_Ceph_Inside_Kubernetes_-_Travis_Nielsen_IBM
tags:
- Ceph
- Kubernetes
- Rook
...
#### 会议概述
Travis Nielsen作为Ceph项目的主要维护者和IBM存储团队的一员，在会上分享了在Kubernetes环境中部署Ceph的重要性及方法。他探讨了Ceph与Kubernetes结合的优势，并介绍了Rook在其中的管理作用。

#### 讨论的主要议题
1. **Kubernetes与存储的需求**：
   - Kubernetes原生并非为存储设计，但部署应用时需要存储支持。
   - 传统上，存储通常是外部的，在私有数据中心使用云提供商存储存在限制。

2. **Ceph在Kubernetes中的部署**：
   - Ceph是企业级的存储解决方案，提供对象、块和文件系统存储。
   - Rook作为管理层，专门用于在Kubernetes中部署和管理Ceph。

3. **Rook的功能与优势**：
   - Rook使用Operator模式，通过自定义资源定义（CRDs）实现Ceph的自动化部署和管理。
   - Rook自2018年12月宣布稳定，已在多个生产环境中验证。
