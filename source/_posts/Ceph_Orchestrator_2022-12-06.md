...
##### 1. OpenStack Tracker 更新
- **背景**：讨论了在Ceph中定制每个OSD的crash设备类的问题。SafeTangible使用批处理模式，通过传递OSD列表来准备和激活设备。
- **当前问题**：CI环境中为SSD创建crash层次结构并应用规则时，由于缺乏对不同crash设备类的支持，无法完全复制过去的功能。
- **解决方案**：
  - 创建了一个新的tracker来比较过去和现在的实现。
  - 讨论了是否引入不同的语法为设备类，或优化现有路径处理以支持每个OSD的设备类。
...
- 讨论团队过渡问题，以及如何更好地支持新成员和处理项目中的问题。
- 更新测试套件，处理剩余的小问题。
