...
1. **Ceph存储系统开发进展**
   - Tommy上周专注于处理异步another tree，并审查了Ritek编写的blue storylines。
   - 本周计划完成heartbeat的审查，并处理工程师Stem提交的replicas by place。

2. **技术问题讨论**
   - **缓存依赖追踪**：Tommy正在为缓存添加支持，以跟踪依赖关系并确保所有父级extent被固定，确保逻辑extent（如extent tree或oh no tree extent）在内存中时，其所有映射页也在内存中。
...
3. **后续行动计划**
   - **Crimson自动化测试**：Tommy计划确保Crimson的自动化测试工作，以便后续可以自信地进行性能优化。
