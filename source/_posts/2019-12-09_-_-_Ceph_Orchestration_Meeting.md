...
updated: 2019-12-10
---
在2019年12月9日的Ceph Orchestration会议中，与会人员讨论了多个与Ceph分布式存储系统相关的关键议题。以下是对会议主要内容的总结：

### 关键细节
- **SSH Orchestrator监控**：Paul开始创建SSH Orchestrator的监控功能，会议参与者对此表示赞赏并计划进行审查。
- **多驱动组命名**：讨论了支持多个驱动组的情况，并建议为每个驱动组命名以便更好地理解和描述其用途。
- **LVM与EVS兼容性**：Sebastian提到LVM与EVS不兼容的问题，建议考虑不使用LVM。
- **系统单元和容器化**：讨论了系统单元和容器化的问题，特别是容器关闭时的清理工作。
- **Python 3迁移**：计划在接下来的一周内进行Python 3的迁移。
- **OMAP转换问题**：在升级到Octopus版本时，发现了OMAP使用统计信息的不同消息，讨论了如何处理这一问题。
...
### 决定事项
- **驱动组命名**：决定为每个驱动组命名以便更好地理解和描述其用途。
- **LVM的使用**：考虑不使用LVM，以避免与EVS的兼容性问题。
- **系统单元和容器化的清理工作**：决定需要一个包装器来处理容器关闭时的清理工作。
- **Python 3迁移**：计划在接下来的一周内进行Python 3的迁移。
...
会议讨论了多个重要议题，并做出了相应的决定和后续行动计划。所有参与者将继续推进这些任务，以确保项目的顺利进行。