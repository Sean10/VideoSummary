...
- 视频总结
date: 2021-04-20
subtitle: Ceph_Developer_Monthly_2021-03-03
tags:
- Ceph
- 分布式存储
- 性能优化
...
1. **Ceph RGW性能优化与配置简化**
   - **讨论内容**：
     - 讨论了通过预设的Erasure Coded（EC）配置文件简化用户配置，提高性能和易用性。
     - 探讨了自动加载预设配置文件，并让用户直接使用的可能性。
     - 讨论了通过RGW客户端根据不同EC配置文件动态调整参数，优化性能。
     - 提出了为特定池设置提示或属性，以优化存储分配和管理的想法。
   - **决定事项**：
     - 需要进一步讨论和定义具体的EC配置文件参数。
     - 需要研究如何在RGW和BlueStore中实现这些优化策略。
   - **后续行动计划**：
     - 创建跟踪问题（tracker issues）和任务分解，具体实施优化策略。
     - 与开发团队合作，确保优化策略的技术可行性和实施细节。

2. **构建与测试优化**
   - **讨论内容**：
     - 讨论了优化测试执行和提高测试覆盖率，包括改进测试的并行执行和减少测试间的重复。
     - 探讨了减少测试环境的设置和重置时间，以及减少日志收集的开销。
