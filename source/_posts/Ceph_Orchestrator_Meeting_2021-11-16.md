...
- 视频总结
date: 2021-11-17
subtitle: Ceph_Orchestrator_Meeting_2021-11-16
tags:
- Ceph
- 分布式存储
- Orchestrator
...
1. **Cephalocon 论文征集**
   - 论文征集截止日期为12月10日。
   - 鼓励团队成员提交关于安全性和NSF ADM文档的演讲提案。
   - 讨论了为不同开发领域（如FDM、Flu Culture系统）设置专门会议环节的可能性。

2. **大型集群部署计划**
   - 计划在澳大利亚珀斯的Poise超级计算中心部署大规模的SF ADM集群。
   - 讨论了如何有效组织和利用时间，避免部署过程中的浪费。
   - 提出了使用IRC或Matrix作为通信渠道，确保信息的连续性和可访问性。
...
#### 决定事项
- 向Cephalocon组织者提出建议，为不同的开发领域设置专门的会议环节。
