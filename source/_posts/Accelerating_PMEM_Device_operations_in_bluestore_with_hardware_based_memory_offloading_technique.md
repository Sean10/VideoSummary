...
1. **背景与问题介绍**
   - 随着非易失性内存设备（PMEM）的普及，如Intel Optane内存，数据中心中CPU资源在操作这些设备时成为瓶颈。
   - 使用应用程序直接模式时，CPU需要处理更多的读写操作，导致资源紧张。
   - 持久内存设备缺乏DMA直接内存访问能力，导致CPU负载增加。

2. **提出的解决方案**
   - 利用硬件卸载技术，如Intel的io80、iox和DSA（数据流加速器），来减轻CPU的负担。
   - DSA是一种高性能的数据复制和转换加速器，将用于优化数据移动和其他操作。
...
4. **结论与未来工作**
   - 通过硬件卸载技术，特别是利用DSA和DML库，可以有效减轻CPU在持久内存操作中的压力。
   - 未来工作包括优化Ceph存储系统中的持久内存操作，并探索其他卸载设备如FPGA的应用。
