...
1. **Orchestrator CLI 简化**:
   - 讨论了通过参数和 YAML 文件两种方式创建新服务的问题，决定简化 CLI，避免两者混用。
   - 计划移除不再使用的特定参数版本。
...
3. **测试和集成**:
   - 讨论了将 Orchestrator 测试集成到 Rook CI/CD 的工作进展。
   - 提到了使用 Ceph-Demon 模块简化虚拟机设置的方法，计划分享给团队。
   - 讨论了将 Safe ADM 方法用于更广泛的测试，特别是希望将 Dashboard 集成到 Safe ADM 中。

4. **Safe ADM 和 Tautology 集成**:
   - 讨论了 Safe ADM 与 Tautology 的集成，以及如何提高测试的覆盖率和可靠性。
   - 提到了一个关于 RGW 的 bug，如果 Safe ADM 中有相应的测试，可能会更早发现。

5. **Safe Deploy 的使用**:
   - 讨论了 Safe Deploy 的使用情况，建议不再关注。
