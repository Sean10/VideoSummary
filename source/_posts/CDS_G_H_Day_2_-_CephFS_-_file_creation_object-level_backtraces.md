...
categories:
- "视频总结"
subtitle: CDS_G_H_Day_2_-_CephFS_-_file_creation_object-level_backtraces
---

### 会议纪要

**会议主题**： Ceph 文件系统（CephFS）文件创建与对象级回溯讨论
...
1. **文件创建和对象级回溯问题**：
   ...
   - 探讨了异步文件创建的可行性，包括如何处理客户端失败和确保文件属性正确。
...
- 需要考虑将回溯信息存储在数据池中，但需注意数据池迁移的情况。
- 考虑使用键值存储来存储回溯信息，以提高效率。
