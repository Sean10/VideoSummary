...
- 视频总结
date: 2020-05-13
subtitle: 2020-05-12_-_-_Crimson_SeaStor_OSD_Weekly_Meeting
tags:
- Ceph
- 分布式存储
...
1. **单元测试与代码重构**
   - **发言人：[姓名]**
   - **内容摘要：** 正在增加单元测试，并计划基于现有能力重写变异和复制逻辑。采用基于树的实现方式来处理分配和释放操作。讨论了PR 334492的相关内容，将在会议后进行详细审查。

2. **代码更新与功能实现**
   - **发言人：[姓名]**
   - **内容摘要：** 更新了exact map tree的实现，正在尝试理解管理器接口并进行树形编码。讨论了与现有代码的相似性和可能的借用。
...
4. **代码审查与项目进展**
   - **发言人：[姓名]**
   - **内容摘要：** 审查了一个PR，认为改动不大，但方向正确。讨论了内存泄漏问题，预计本周解决。

5. **服务理解与状态机代码迁移**
   - **发言人：[姓名]**
   - **内容摘要：** 通过阅读代码来理解服务，以便实际迁移状态机代码。计划通过提问来进一步理解服务和基础设施。
