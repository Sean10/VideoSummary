...
- 视频总结
date: 2021-08-21
subtitle: Ceph_Performance_Meeting_2021-08-05
tags:
- Ceph
- 性能优化
- 蓝店 (BlueStore)
...
#### 关键细节
- **新PR**: 本次会议讨论了一个新的PR，该PR涉及MDS锁的改进，将锁从非公平互斥锁更改为公平互斥锁，旨在改善等待时间较长的请求的处理，避免资源饥饿。
- **已关闭PR**: Kifu合并了一个关于PG日志的更改PR，具体是关于回滚信息的修剪。
- **更新PR**:  多个PR正在积极审查和开发中，包括RGW跟踪工作、缓冲区列表的PR以及TTL缓存实现的长期工作。
- **性能问题**: Red Hat内部工作组在基于Pacific的新存储版本中发现了性能提升，但NVMe基础的DB和WAL分区在OSD中出现了显著的写放大问题，导致SSD驱动器的工作负载比之前版本更重。
...
#### 结论
会议讨论了Ceph存储系统中关于MDS锁改进和BlueStore中写放大问题等多个开发和性能优化问题。决定进行更多测试和代码审查，以解决这些问题并优化系统性能。

### 改进点
- 增加了关于BlueStore中写放大问题的详细描述。
- 优化了关于性能测试和代码审查的行动计划。
- 确保保留计算机科学/CEPH相关领域的英文原文关键词。