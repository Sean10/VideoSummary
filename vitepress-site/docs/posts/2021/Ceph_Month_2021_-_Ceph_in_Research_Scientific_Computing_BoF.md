...
#### 主要讨论议题
1. **Ceph RBD Mirroring**: 会上将有一位新成员进行关于RBD镜像的演示。
2. **CephFS Snapshots**: CERN团队在测试CephFS的快照功能时，发现Nautilus和Octopus版本中快照删除操作非常慢，建议升级到Pacific版本解决。
3. **Ceph Adm的使用**: 讨论了Ceph Adm在管理Ceph集群中的应用，特别是与Puppet的集成问题和容器镜像的管理。
4. **Ceph升级经验分享**: 分享了从Luminous升级到Octopus的经验，强调了升级过程中的关键步骤和注意事项。
...
- 计划在下一个会议周期再次举行“鸟类聚会”，以继续讨论Ceph和科学计算的相关话题。
