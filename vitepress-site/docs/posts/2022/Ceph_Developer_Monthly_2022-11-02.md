...
1. **Crimson OSD 用户界面保护措施**
   - 讨论了为防止用户意外启动 Crimson OSD 而导致生产集群复杂化的问题。
   - 引入了两种保护措施：编辑 Crimson 实验性功能标志和设置允许 Crimson OSD 映射标志。
   - 这些措施旨在防止用户在不完全支持 Crimson OSD 的功能时意外使用它们。

2. **Crimson 池的设计和功能限制**
   - 讨论了未来可能需要在同一集群中混合使用 Crimson OSD 和常规 OSD 的情况。
   - 介绍了基于每个池的设置，限制了 Crimson 池的一些功能，如更改 PG 数量或使用 tiers。

3. **文档和未来支持**
   - 认识到当前文档中对 Crimson 支持功能的描述不足，计划在未来的 Reef 文档中详细说明支持的功能。
